...
---
# Adding support to LIST and DELETE similar messages
QUERY: Can an outlook plug in search for other messages within the same folder? For example, after unsubscribing, the script could then look for all messages with the same sender SMTP address (and same recipient SMPT) and display a list of those messages with dates and subjects and ask if the user wants to delete these?


Yes. That workflow is feasible, but **not with Office.js alone**.

Microsoft’s current guidance is explicit: when an Outlook add-in needs to find other messages in the mailbox—for example, messages from the same sender—it should use **Microsoft Graph**. Office.js is good for the currently open item; Graph is the right API for mailbox-wide or folder-level searches. ([Microsoft Learn][1])

For your scenario, I would design it like this:

1. User clicks **Unsubscribe**.
2. Our existing unsubscribe logic completes.
3. The add-in reads from the current message:

   * sender SMTP address
   * recipient SMTP address(es)
   * `internetMessageId`
4. Using Microsoft Graph, locate that current message and get its `parentFolderId`.
5. Query only that folder for messages matching the sender and recipient.
6. Return just:
   ...
   * recipient
   * Graph message ID
7. Show those results in a dialog/task pane with checkboxes.
8. User selects messages and clicks **Delete Selected**.
9. Graph deletes those selected messages.

Graph supports listing messages within a specific folder using:

```text
GET /me/mailFolders/{folder-id}/messages
```

and you can limit the returned properties using `$select` to keep the query efficient. ([Microsoft Learn][2])

A result UI could look like:
...
☑ Sep 18, 2026   Fall Newsletter
☑ Sep 11, 2026   Weekly Update
☑ Sep 04, 2026   September Offers
☐ Aug 28, 2026   Welcome to Newsletter
...
This would require a meaningful change to our add-in.

Reading/searching messages can use delegated Graph mail permissions such as `Mail.Read` or `Mail.ReadBasic`, but deleting messages requires:

```text
Mail.ReadWrite
```

Microsoft defines delegated `Mail.ReadWrite` as permission to create, read, update, and delete email in the signed-in user's mailbox; it does **not** grant send-mail permission. ([Microsoft Learn][3])

Deleting an individual message is then simply:
...
```http
DELETE /me/mailFolders/{folder-id}/messages/{message-id}
```

and Graph requires `Mail.ReadWrite` for that operation. ([Microsoft Learn][4])

### Authentication

Our current add-in doesn't authenticate to Graph at all, so we would need to add Microsoft authentication.

Microsoft's current recommended architecture is **Nested App Authentication (NAA)** using MSAL.js. NAA lets an Outlook add-in obtain a Graph access token for the signed-in user and generally avoids a separate sign-in redirect once everything is configured. ([Microsoft Learn][5])

That means we would need an **Entra ID application registration** for the add-in and request delegated:

```text
Mail.ReadWrite
```

This is the biggest architectural step beyond what we've built so far.

### Identifying the current folder

This is also possible.

Office.js gives us the currently selected message's:
...
Microsoft documents `itemId` as an EWS-format ID and notes that it must be converted before use with REST-style APIs; `internetMessageId` is also available directly in Message Read mode. ([Microsoft Learn][6])

Once we resolve that current message in Graph, the Graph message object provides the information needed to identify its folder, including its `parentFolderId`. That gives us the “same folder only” behavior rather than scanning the whole mailbox. ([Microsoft Learn][7])

### Sender + recipient matching

I would be slightly conservative here.

Suppose the current message is:

```text
From: newsletter@example.com
To: awsles@example.com
```

We should search for messages where:

```text
From SMTP = newsletter@example.com
AND
recipient list contains awsles@example.com
AND
parentFolderId = current folder
```

That is better than sender-only matching because newsletters sometimes use the same sending address for multiple aliases or mailboxes.

I would also **exclude the currently open message** from the cleanup list unless you specifically want it included.

### One UX recommendation

I would not automatically delete anything after unsubscribe.
...
"Unsubscribe request submitted"
    ↓
"14 other messages from this sender were found"
    ↓
[ Review Messages ]
    ↓
checkbox list
...
That makes the cleanup feature useful without making unsubscribe unexpectedly destructive.

### The main architectural change

Right now our add-in is essentially:
...
Microsoft even uses “identify messages in a user's mailbox that originated from the same sender” as an example of when an Outlook add-in should use Graph rather than Office.js. ([Microsoft Learn][1])

So yes: **this is a very natural next feature for this add-in.** It is technically straightforward once Graph authentication is added, but adding NAA/Entra registration is a substantially bigger step than the current static GitHub Pages add-in.

[1]: https://learn.microsoft.com/en-us/office/dev/add-ins/outlook/microsoft-graph "Use the Microsoft Graph REST API from an Outlook add-in - Office Add-ins | Microsoft Learn"
[2]: https://learn.microsoft.com/en-us/graph/api/user-list-messages?view=graph-rest-1.0&utm_source=chatgpt.com "List messages - Microsoft Graph v1.0 | Microsoft Learn"
[3]: https://learn.microsoft.com/en-us/graph/permissions-reference?utm_source=chatgpt.com "Microsoft Graph permissions reference - Microsoft Graph | Microsoft Learn"
[4]: https://learn.microsoft.com/et-ee/graph/api/message-delete?view=graph-rest-1.0&utm_source=chatgpt.com "Delete message - Microsoft Graph v1.0 | Microsoft Learn"
[5]: https://learn.microsoft.com/en-us/office/dev/add-ins/outlook/microsoft-graph?utm_source=chatgpt.com "Use the Microsoft Graph REST API from an Outlook add-in - Office Add-ins | Microsoft Learn"
[6]: https://learn.microsoft.com/en-us/javascript/api/outlook/office.messageread?view=outlook-js-preview&utm_source=chatgpt.com "Office.MessageRead interface - Office Add-ins | Microsoft Learn"
[7]: https://learn.microsoft.com/en-us/graph/api/resources/message?preserve-view=true&view=graph-rest-1.0&utm_source=chatgpt.com "message resource type - Microsoft Graph v1.0 | Microsoft Learn"
